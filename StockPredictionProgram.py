...
# Load Data from file
trainStock1 = pd.read_csv(PreTrainStock1, header=None, delimiter='\t', names=["Date", "Open", "High", "Low", "Close", "Adj Close", "Volume"])
...
def runTrainPredict(StockDict, StockKey, StockColumn, fileName):
    ...
    resultEpoch = []
    resultLoss = []    
    X_train, y_train, train_data, test_data, TestStock, trainScalar, hidden_state =  TrainStockPrepare(StockDict,StockKey,StockColumn) 
    ...
    for lstmLoopi in range(TestPredictDay):  
        LSTMStockPredictionResultList.append(PrePredictOut[lstmLoopi+1].item())
    print("")
    print("LSTMStockPredictionResultList:",LSTMStockPredictionResultList)
...
#Run Main LSTM Train Program
def returnLSTMTrainProgramStockPriceResult():
    runLSTMTrainProgram()
    #print("LSTMStockPredictionResultList:",LSTMStockPredictionResultList)
    return LSTMStockPredictionResultList
